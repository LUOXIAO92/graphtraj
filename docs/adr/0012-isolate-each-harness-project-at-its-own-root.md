...
existing Source Repository checkout at
...
the repository and chooses its directory name before setup, then identifies
that directory to the interactive setup command. Setup does not clone a
repository. It runs from the Harness Project Root and must not discover, group,
authorize, or share state with neighboring projects merely because they have
the same filesystem parent.
...
and role definitions use the repository-relative `.codex/` directory, and
...
resources in Git is independent of keeping Runner worktrees and delivery state
outside Git.
...
├── <repository-directory>/                  # primary main checkout
│   └── .git/
└── state/
```
