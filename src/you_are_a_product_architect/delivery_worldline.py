"""Atomic Delivery Worldline append and readable YAML projection."""
...
@click.group()
def worldline() -> None:
    """Record and project one Delivery Run's explicit Worldline events."""
...
def append_command(run_root: Path, run_id: str, event_file: Path) -> None:
    """Append one explicit YAML event supplied by the Delivery State Agent."""

    try:
        event = yaml.safe_load(event_file.read_text(encoding="utf-8"))
        if not isinstance(event, dict):
            raise ValueError("event file must contain one mapping")
        recorded = append_worldline_event(run_root, run_id, event)
...
def project_command(run_root: Path) -> None:
    """Regenerate ledger.yml from the canonical append-only journal."""
...
def append_worldline_event(
    ...
    """Append one explicit event and regenerate its deterministic projection."""
    ...
    try:
        ...
        with os.fdopen(descriptor, "r+", encoding="utf-8", closefd=False) as stream:
            ...
            events = [*existing, normalized]
            _write_projection(run_root, run_id, events)
        return normalized
    finally:
...
def project_worldline(run_root: Path) -> Path:
    """Regenerate ledger.yml solely from one canonical worldline.jsonl."""
    ...
    try:
        ...
        return _write_projection(run_root, run_id, events)
    finally:
        fcntl.flock(descriptor, fcntl.LOCK_UN)
        os.close(descriptor)
...
def _validate_supplied_event(event: Mapping[str, Any]) -> None:
    ...
    if kind.startswith("agent-turn-"):
        ...
        if not isinstance(event["turn"], int) or isinstance(event["turn"], bool):
