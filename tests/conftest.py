...
def fake_codex(tmp_path: Path) -> FakeCodex:
    ...
    script = "#!{0}\n".format(sys.executable) + (
        ...
        "    commands = ['agent-runner --help', 'codex exec hello', 'python -m graphtraj.agent_runner', 'pwd']\n"
        "    report = {'engineer-junior': 'engineer.md', 'team-leader': 'leader.md'}.get(os.environ['GRAPHTRAJ_ROLE'])\n"
        "    if report:\n"
        "        report_path = 'teams/{0}/rounds/{1}/{2}'.format(os.environ['GRAPHTRAJ_TEAM_GENERATION'], os.environ['GRAPHTRAJ_TEAM_ROUND'], report)\n"
        "        commands.extend(('touch .state/' + report_path, 'touch ' + shlex.quote(str(Path(os.environ['GRAPHTRAJ_EVIDENCE']) / report_path))))\n"
