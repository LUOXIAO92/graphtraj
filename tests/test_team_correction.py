...
if role == 'delivery-state':
    ...
    if not (round_dir / 'engineer.md').exists():
        ...
        if expanded and (reviewed or os.environ['CORRECTION_TIMING'] == 'before-review'):
...
@pytest.mark.parametrize('timing', ['before-review', 'after-review'])
def test_installed_team_corrects_engineer_and_reviewer_in_same_round(
    installed_commands, temporary_git_repository, fake_codex, tmp_path, timing
):
